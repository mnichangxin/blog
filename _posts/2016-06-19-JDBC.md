...
* 在程序中不需要直接去访问实现了 Driver 接口的类，而是由驱动程序管理器类(java.sql.DriverManager)去调用这些 Driver实现
...
1. Java.sql.PreparedSatement 用于执行预编译的sql声明
2. Java.sql.CallableStatement用于执行数据库中存储过程的调用
